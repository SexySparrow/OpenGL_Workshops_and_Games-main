...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	glUniform3fv(loc_eye_pos, 1, glm::value_ptr(eye_position));

	// TODO: Set material property uniforms (shininess, kd, ks, object color)
	GLint loc_material = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(loc_material, materialShininess);

	GLint loc_material_kd = glGetUniformLocation(shader->program, "material_kd");  // diffuse light
	glUniform1f(loc_material_kd, materialKd);

	GLint loc_material_ks = glGetUniformLocation(shader->program, "material_ks");  // specular light
	glUniform1f(loc_material_ks, materialKs);

	GLint loc_object = glGetUniformLocation(shader->program, "object_color");
